...
d3.csv("data/rq3.csv", typeConversor, function(error, data) {
    ...
    var firstComment = data[0].first_comment;
    var firstCommentCollaborator = data[0].first_comment_collaborator;
    var timeToMerge = data[0].time_to_merge;
    var timeToClose = data[0].time_to_close;
    var percClosed = data[0].perc_closed;
    var percMerged = data[0].perc_merged;
    var percOpen = data[0].perc_open;
...
function draw(container, firstComment, firstCommentCollaborator, timeToMerge, timeToClose, percClosed, percMerged, percOpen) {
    ...
    var scaley = d3.scale.ordinal()
        .domain([1, 2, 3]).rangePoints([0+2*heightExtension, height-2*heightExtension], 0).range();
    ...
    var upperScale = d3.scale.ordinal()
        .domain([1, 2, 3, 4]).rangePoints([d3.max([firstComment, firstCommentCollaborator]), timeToMerge], 0).range();
    ...
    var lowerScale = d3.scale.ordinal()
        .domain([1, 2, 3, 4]).rangePoints([d3.max([firstComment, firstCommentCollaborator]), timeToClose], 0).range();
